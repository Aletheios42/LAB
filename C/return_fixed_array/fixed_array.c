...
char (*get_subtones(const char *color))[TONE_COUNT] {
  ...
  if (strcmp(color, "Rojo") == 0)
    return (
        char(*)[TONE_COUNT])red_tones; // Retorna puntero al arreglo de subtonos
  if (strcmp(color, "Verde") == 0)
    return (char(*)[TONE_COUNT])green_tones;
  if (strcmp(color, "Azul") == 0)
    return (char(*)[TONE_COUNT])blue_tones;
  if (strcmp(color, "Amarillo") == 0)
    return (char(*)[TONE_COUNT])yellow_tones;
  if (strcmp(color, "Violeta") == 0)
    return (char(*)[TONE_COUNT])violet_tones;
...
int main() {
  ...
  if (index >= 1 && index <= COLOR_COUNT) {
    ...
    char(*subtones)[TONE_COUNT] = get_subtones(selected_color);
